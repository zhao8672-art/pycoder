...
def detect_environment(project_path: str | Path | None = None) -> Any | None:
    """检测项目环境（兼容性别名）"""
    try:
        from pycoder.python.env_detector import EnvironmentInfo, detect_environment as _detect
...
def generate_agents_md(
    ...
    if env:
        ...
        sections.append("")
        sections.append(f"- **项目名称**: pycode")
        sections.append(f"- **主要语言**: Python 3.x")
        sections.append(f"- **包管理器**: pip")
        sections.append(f"- **项目类型**: Web 应用")
        sections.append(f"- **框架**: FastAPI, pandas, NumPy, Matplotlib, Streamlit, pytest, Textual, Rich")
        sections.append("")
    ...
    sections.append("4. **现代语法** — 优先使用 Python 3.10+ 特性（match/case, `|` 联合类型, walrus `:=`）")
    ...
    sections.append("- 提交格式: `feat: xxx` / `fix: xxx` / `docs: xxx` / `refactor: xxx` / `test: xxx`")
    ...
    sections.append("")
    sections.append("- `.git/hooks/post-commit` 已安装，每次 `git commit` 后自动执行 `git push origin master`")
