...
def register(registry: Any) -> None:
    _reg(registry, "tools.env.python", "Python 环境",
         "扫描并列出所有可用的 Python 虚拟环境和版本信息",
         {}, [], _handle_python_env)

    _reg(registry, "tools.env.docker_status", "Docker 状态",
         "检查 Docker 执行后端的可用性和状态",
         {}, [], _handle_docker_status)

    _reg(registry, "tools.env.docker_execute", "Docker 执行",
         "在隔离的 Docker 容器中安全执行 Python 代码",
         {"code": {"type": "string"}, "timeout": {"type": "number", "default": 30}},
         ["code"], _handle_docker_execute)
...
async def _handle_python_env(params: dict, context: dict) -> dict:
    ...
    for name in (".venv", "venv", "env"):
        p = cwd / name
        if p.exists():
            envs.append({"name": name, "path": str(p), "type": "venv",
                          "active": venv == str(p)})
    v = f"{sys.version_info.major}.{sys.version_info.minor}"
    return {"success": True, "environments": envs, "python_path": sys.executable,
            "python_version": v}
...
async def _handle_docker_execute(params: dict, context: dict) -> dict:
    ...
    result = await backend.execute(
        params["code"], timeout=params.get("timeout", 30))
