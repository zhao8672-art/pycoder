...
def register(registry: Any) -> None:
    ...
    _reg(
        ...
        ["language", "code"],
        _handle_execute_multilang,
    )

    _reg(
        registry,
        "tools.exec.debug_python",
        "调试Python",
        ...
        ["code"],
        _handle_debug_python,
    )

    _reg(
        registry,
        "tools.exec.profile_python",
        "性能分析",
        "用 cProfile 分析 Python 代码性能，返回热点函数和调用链",
        {
            "code": {"type": "string"},
            "sort_by": {"type": "string", "default": "cumtime"},
            "timeout": {"type": "number", "default": 30},
        },
        ["code"],
        _handle_profile_python,
    )

    # list_languages — 只读工具
    registry.register(
        CapabilityDefinition(
            id="tools.env.languages",
            name="列出语言运行时",
...
async def _handle_execute_python(params: dict, context: dict) -> dict:
    ...
    _run_in_subprocess = _mod._run_in_subprocess
...
async def _handle_execute_code(params: dict, context: dict) -> dict:
    ...
    try:
        if language == "python":
            r = sp.run(["python", "-c", code], capture_output=True, text=True, timeout=timeout)
            return _mkres(r.returncode == 0, r.stdout, r.stderr, language)
        if language == "javascript":
            r = sp.run(["node", "-e", code], capture_output=True, text=True, timeout=timeout)
            return _mkres(r.returncode == 0, r.stdout, r.stderr, language)
        if language == "shell":
            ...
            tf.close()
            try:
                r = sp.run(["bash", tf.name], capture_output=True, text=True, timeout=timeout)
                return _mkres(r.returncode == 0, r.stdout, r.stderr, language)
            finally:
...
async def _handle_execute_multilang(params: dict, context: dict) -> dict:
    ...
    return await execute_multilang(params["language"], params["code"], params.get("timeout", 30))
