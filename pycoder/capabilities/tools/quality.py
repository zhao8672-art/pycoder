...
def register(registry: Any) -> None:
    _reg(registry, "tools.quality.code_review", "代码审查",
         "对代码片段进行静态分析，返回质量评分和问题列表",
         {"code": {"type": "string"}, "language": {"type": "string", "default": "python"}},
         ["code"], _handle_code_review)

    _reg(registry, "tools.quality.format_code", "格式化代码",
         "用 black/ruff/isort 自动格式化 Python 代码",
         {"code": {"type": "string"},
          "style": {"type": "string", "enum": ["black", "ruff", "isort"], "default": "black"}},
         ["code"], _handle_format_code)

    _reg(registry, "tools.quality.security_scan", "安全扫描",
         "扫描项目依赖中的已知安全漏洞",
         {"path": {"type": "string", "default": "."}},
         [], _handle_security_scan)

    _reg(registry, "tools.quality.dependency_analysis", "依赖分析",
         "分析项目的 Python 依赖树",
         {"path": {"type": "string", "default": "."}},
...
async def _handle_security_scan(params: dict, context: dict) -> dict:
    ...
    return {"success": True, "total_deps": total,
            "vulnerabilities": [], "summary": f"扫描了 {total} 个依赖"}
