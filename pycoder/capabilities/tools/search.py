...
def register(registry: Any) -> None:
    _reg(registry, "tools.search.text", "文本搜索",
         "在项目中搜索文本内容（关键词匹配）",
         {"query": {"type": "string", "description": "搜索关键词"},
          "include_pattern": {"type": "string", "default": "**/*.py"},
          "max_results": {"type": "number", "default": 20}},
         ["query"], _handle_search)

    _reg(registry, "tools.search.quick_open", "快捷打开",
        ...
        try:
            ...
            for i, line in enumerate(content.split("\n"), 1):
                if query.lower() in line.lower():
                    results.append({"file": str(f.relative_to(root)),
    ...
    for f in list(Path.cwd().rglob("*"))[:500]:
        ...
        if f.is_file() and f.suffix in (".py", ".ts", ".tsx", ".js", ".md", ".json", ".toml"):
            ...
            if not query or query in rel.lower():
                results.append({"path": rel, "name": f.name,
                                "size": f.stat().st_size})
                if len(results) >= max_results:
                    break
    return {"success": True, "results": results, "total": len(results)}
