...
def register(registry: Any) -> None:
    ...
    for cid, name, desc, schema in [
        (
            "tools.marketplace.skills_search",
            "Skills 搜索",
        ...
        (
            "tools.marketplace.skills_recommendations",
            "Skills 推荐",
        ...
        (
            "tools.marketplace.skills_trending",
            "Skills 热门",
            "实时热门技能榜单",
            {"limit": {"type": "integer", "default": 20}},
        ),
        ("tools.marketplace.skills_stats", "Skills 统计", "Skills 市场统计仪表板", {}),
        ("tools.marketplace.skills_categories", "Skills 分类", "列出 Skills 所有分类", {}),
        (
            "tools.marketplace.skills_detail",
            "Skills 详情",
            "获取 Skills 技能详情",
    ...
    _reg(
        registry,
        "tools.marketplace.skills_update",
        "Skills 更新",
    ...
    _reg(
        registry,
        "tools.marketplace.extensions_search",
        "扩展搜索",
        "搜索可用扩展市场",
        {
            "query": {"type": "string", "default": ""},
    ...
    _reg(
        registry,
        "tools.marketplace.extensions_installed",
        "已安装扩展",
        "列出所有已安装的扩展及其状态",
        {},
    ...
    _reg(
        registry,
        "tools.marketplace.extensions_install",
        "安装扩展",
        "安装一个扩展",
        {"id": {"type": "string"}},
        _handle_extensions_install,
    )
    _reg(
        registry,
        "tools.marketplace.extensions_uninstall",
        "卸载扩展",
        "卸载一个已安装的扩展",
        {"id": {"type": "string"}},
        _handle_extensions_uninstall,
    )
    _reg(
        registry,
        "tools.marketplace.extensions_refresh",
        "刷新扩展",
        "强制刷新扩展市场缓存",
        {},
    ...
    _reg(
        registry,
        "tools.marketplace.system_upgrade",
        "系统升级",
        "系统升级：检查更新、执行升级、健康检查",
...
def _reg(registry, cid, name, desc, schema, handler):
    required = (
        [k for k, v in schema.items() if isinstance(v, dict) and v.get("required")]
        if schema
        else []
    )
    registry.register(
        CapabilityDefinition(
            ...
            side_effects=[
                (
                    SideEffect.NETWORK
                    if "sync" in cid or "update" in cid or "install" in cid or "search" in cid
