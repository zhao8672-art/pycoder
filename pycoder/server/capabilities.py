...
def generate_capabilities() -> str:
    ...
    sections.append(_build_lsp_section())
    sections.append(_build_self_evo_section())
    sections.append(_build_modules_index())
    sections.append(_build_reasoning_config())
    sections.append(_build_testing_section())
    sections.append(_build_code_execution_section())
    sections.append(_build_git_section())
    sections.append(_build_database_section())
...
def _build_header() -> str:
    return f"""# PyCoder v{__version__} 能力清单
...
def _build_self_evo_section() -> str:
    """构建自我进化引擎章节"""
    return f"""
...
def _build_reasoning_config() -> str:
    """构建推理配置章节"""
    return """
## 6. DeepSeek 推理控制
...
def _build_testing_section() -> str:
    """构建测试与 CI/CD 章节"""
    return """
## 7. 测试与 CI/CD
...
def _build_code_execution_section() -> str:
    """构建代码执行与调试章节"""
    return """
## 8. 代码执行与调试
...
def _build_permissions_section() -> str:
    """构建权限控制章节"""
    return """
## 6. 权限控制
