    ...
    async def list_remote_tools(self, server_name: str) -> list[dict]:
        ...
        try:
            ...
            return [{"name": t.name, "description": t.description or "",
                     "input_schema": t.inputSchema or {}, "server": server_name}
                    for t in tools.tools]
        except Exception as e:
            log.warning("mcp_list_remote_failed", server=server_name, error=str(e))
            return []

    async def call_remote_tool(self, server_name: str, tool_name: str,
                               args: dict) -> MCPCallResult:
        server = self._servers.get(server_name)
        if not server:
            return MCPCallResult(success=False, error=f"Server {server_name} 未连接", tool=tool_name)
        try:
