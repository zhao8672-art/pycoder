...
class GlobalMemory:
    ...
    def __init__(self, storage_dir: Path):
        ...
        if _CHROMA_AVAILABLE:
            try:
                ...
                logger.info(
                    "global_memory_chroma_initialized path=%s", self._storage_dir / "chroma"
                )
    ...
    def _get_sqlite_conn(self) -> sqlite3.Connection:
        ...
        conn.execute("""CREATE TABLE IF NOT EXISTS global_entries (
                id TEXT PRIMARY KEY, key TEXT NOT NULL, content TEXT NOT NULL,
                metadata TEXT DEFAULT '{}', timestamp REAL NOT NULL, ttl REAL
            )""")
        conn.execute("""CREATE INDEX IF NOT EXISTS idx_global_entries_key ON global_entries(key)""")
        conn.commit()
        return conn
