...
class TestGetHotspots:
    ...
    def test_extracts_from_experience_buffer(self, isolated_pe: PatternExtractor):
        exps = [
            ...
            StubExperience(
                file_paths=["a.py"], error_message="err2", outcome="failure", timestamp=time.time()
            ),
            StubExperience(
                file_paths=["a.py"], error_message="err3", outcome="failure", timestamp=time.time()
            ),
    ...
    def test_limits_to_top_n(self, isolated_pe: PatternExtractor):
        ...
        kb = StubKnowledgeBase(
            hotspots=kb_hotspots, risk_map={f"file{i}.py": 10.0 for i in range(15)}
        )
    ...
    def test_orders_by_error_count_desc(self, isolated_pe: PatternExtractor):
        ...
        kb = StubKnowledgeBase(
            hotspots=kb_hotspots, risk_map={"low.py": 10, "high.py": 50, "mid.py": 25}
        )
        result = isolated_pe.get_hotspots(knowledge_base=kb, top_n=10)
...
class TestClusterErrors:
    ...
    def test_clusters_below_min_size_filtered(self, isolated_pe: PatternExtractor):
        ...
        exps = [
            StubExperience(
                error_signature="ClusterA: detail", outcome="success", timestamp=time.time()
            ),
            StubExperience(
                error_signature="ClusterB: detail", outcome="success", timestamp=time.time()
            ),
        ]
        buf = StubExperienceBuffer(exps=exps)
...
class TestGetPatternStats:
    ...
    def test_with_patterns(self, isolated_pe: PatternExtractor):
        isolated_pe._patterns = [
            FixPattern(pattern_id="P1", error_type="ValueError", success_count=8, fail_count=2),
            FixPattern(pattern_id="P2", error_type="TypeError", success_count=5, fail_count=5),
            FixPattern(pattern_id="P3", error_type="ValueError", success_count=10, fail_count=0),
        ]
        stats = isolated_pe.get_pattern_stats()
    ...
    def test_limits_patterns_to_20(self, isolated_pe: PatternExtractor):
        isolated_pe._patterns = [
            FixPattern(pattern_id=f"P{i}", error_type="E", success_count=i + 1, fail_count=0)
            for i in range(30)
        ]
        stats = isolated_pe.get_pattern_stats()
