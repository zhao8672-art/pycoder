...
class TestListSkills:
    ...
    def test_pagination(self, mgr, tmp_path):
        _seed_registry(
            mgr, [{"id": f"s{i}", "name": f"N{i}", "stars": i} for i in range(10)], tmp_path
        )
    ...
    def test_pagination_at_end(self, mgr, tmp_path):
        _seed_registry(mgr, [{"id": f"s{i}", "name": f"N{i}"} for i in range(5)], tmp_path)
...
class TestUpdateAllSkills:
    ...
    def test_update_with_update_available(self, mgr, tmp_path):
        ...
        _seed_registry(
            mgr,
            [
                {"id": "s1", "name": "X", "version": "2.0.0", "description": "d"},
            ],
            tmp_path,
        )
        # 模拟已安装（旧版本）
