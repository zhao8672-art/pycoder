...
def models():
    ...
    return (
        s == 200 and len(d.get("models", [])) > 0,
        f"{len(d.get('models', []))} models, recommended={d.get('recommended_model')}",
    )
...
def skills_list():
    ...
    return s == 200, "ok" if isinstance(d, (list, dict)) else str(d)[:60]
...
def code_exec_python():
    s, d = api("POST", "/api/code/exec", {"code": "print('hello pycoder')", "timeout": 5})
...
def code_exec_shell():
    ...
    s, d = api("POST", "/api/code/run", {"code": "print('hello')", "timeout": 5})
    if s == 200 and isinstance(d, dict):
